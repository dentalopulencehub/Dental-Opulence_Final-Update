...
const MagicCard: React.FC<MagicCardProps> = ({
  ...
  return (
    <div
      ...
      className={`relative z-0 h-full w-full rounded-md p-6 bg-gray-300 dark:bg-gray-700 bg-[radial-gradient(var(--mask-size)_circle_at_var(--mouse-x)_var(--mouse-y),var(--border-color),transparent_100%)] ${className}`
      }
      {...props}
    >
      {children}
      ...
      <div
        className={
          "absolute inset-[1px] -z-20 rounded-md bg-white dark:bg-black/95"
        }
      />
    </div>
  );
