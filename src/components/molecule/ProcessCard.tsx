...
const ProcessCard = ({ processes }: HomeOurServicesType) => {
  return (
    ...
    >
      <div className="xs:w-[394px] w-full  sm:h-[228px] md:w-full h-full bg-[#161616] rounded-lg py-8 px-6 flex flex-col justify-between items-start border border-[#2D2D2D]">
        ...
        <Link href={"/testimonials"}>
          <button className="flex flex-row items-center">
            <span className="text-xs font-Pangram-Regular text-white">
              Our story
            </span>
      ...
      {processes.map((data: any, index: number) => (
        <MagicCard
          key={index}
          className="xs:w-[394px] md:my-0 md:w-full my-5 w-full xs:h-[228px] h-full flex  cursor-pointer flex-col items-start justify-center overflow-hidden p-20 shadow-2xl"
        >
          <Image src={data.image} alt={data.title} />
          <h3 className="font-Pangram-Regular text-2xl text-white mt-4">
            {data.title}
          </h3>
